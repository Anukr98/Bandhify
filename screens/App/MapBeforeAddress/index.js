...
export default class VerifyLocation extends React.Component {
    ...
    componentDidMount() {
        ...
        this.backbuttonAction = BackHandler.addEventListener('hardwareBackPress', this.handleBakcButtonClick)
        Geolocation.getCurrentPosition(position => {
            const { latitude, longitude } = position.coords
            Geocoder.init(keys.GOOGLE_MAPS_API)
            Geocoder.from(params?.item?.latitude ? params?.item?.latitude : latitude, params?.item?.longitude ? params?.item?.longitude : longitude).then(res => {
                ...
                this.setState({
                    latitude: params?.item?.latitude ? params?.item?.latitude : latitude,
                    longitude: params?.item?.longitude ? params?.item?.longitude : longitude,
                    markerCoordinates: {
                        latitude: params?.item?.latitude ? params?.item?.latitude : latitude,
                        longitude: params?.item?.longitude ? params?.item?.longitude : longitude
                    },
                    ...
                    markerAddress: params?.item?.latitude ? `${params?.item?.address_line_1} ${params?.item?.address_line_2} ${params?.item?.address_line_3} ${params?.item?.city?.city}` : address
                })
            })
        }, error => {
            alert(error)
    ...
    handleBakcButtonClick = () => {
        ...
        if(direction === 'Edit' || direction === 'Add')
            ...
            navigation.replace("Checkout", { focus: this.props.route.params.item.address_line_1 })
        return true
    }
    ...
    confirmAddress = () => {
        ...
        this.props.navigation.navigate("AddAddress", { direction, item: params.item ? params.item : null, markerCoordinates })
    ...
    goToInitialPosition = () => {
        ...
        Geocoder.init(keys.GOOGLE_MAPS_API)
        Geocoder.from(this.state.currentLocation.latitude, this.state.currentLocation.longitude).then(res => {
    ...
    render() {
        return <View style = {{ flex: 1, backgroundColor: colors.WHITE }}>
            ...
            <View style = {{ position: "absolute", top: "9%", width: "93%", alignSelf: "center", borderRadius: 7, flex: 1, flexDirection: "row", alignItems: "center" }}>
            ...
            <View style = {{ position: "absolute", left: '5%', top: "3%", backgroundColor: "#ffffff", elevation: 7, borderRadius: 7, width: 40, height: 40  }}>
                <Pressable style = {{ alignItems: "center", justifyContent: "center", height: 40, width: 40 }} onPress = {this.handleBakcButtonClick}>
            ...
            <View style = {{ height: 190, backgroundColor: colors.WHITE, borderTopLeftRadius: 30, borderTopRightRadius: 30, overflow: "hidden", justifyContent: "space-evenly" }}>
                <View style = {{ width: "90%", alignSelf: "center" }}>
                    <View style = {{ marginBottom: "2%" }}><Text style = {{ fontFamily: ColorsText.regular.fontFamily, color: colors.GRAY_TEXT_NEW, fontSize: 12 }}>You can also tap on the map to change your location *</Text></View>
                    <View style = {{ width: "70%" }}><Text style = {{ fontFamily: ColorsText.Medium.fontFamily, color: colors.DUSKY_BLACK_TEXT, letterSpacing: .6 }}>{this.state.markerAddress}</Text></View>
                </View>
                <View style = {{ width: "90%", alignSelf: "center"}}>
                    <TouchableRipple
                        onPress = {() => this.confirmAddress()}
                        rippleColor = {colors.RIPPLE_COLORS.WHITE}
                        style = {{ width: "100%", backgroundColor: colors.ROYAL_BLUE, height: 50, borderRadius: 5, alignItems: "center", justifyContent: "center", alignSelf: "center" }}
                    >
                        <Text style = {{ color: colors.WHITE, fontFamily: ColorsText.Medium.fontFamily, fontSize: 20 }}>Confirm</Text>
                    </TouchableRipple>
                </View>
            </View>
        </View>
