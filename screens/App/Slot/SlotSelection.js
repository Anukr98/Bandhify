  ...
  useEffect(() => {
    ...
    const unsubscribe = navigation.addListener('focus', async () => {
      ...
      for (let i = 0; i < res.length; i++) {
        if (res.item(i).is_subscribable == 1) flag = true;
        else flag = false;
      }
      if (flag) setShowSubscriptionMenu(true);
      ...
      postMethod(
        `${urls.GET_DELIVERY_SLOTS}?shop_id=${res.item(0).shop_id}`,
        object,
        (err, result) => {
        ...
        <TouchableWithoutFeedback onPress={() => selectSlot(item)}>
          <View>
            <Icon
              ...
              type="Ionicons"
              name={
                item.slot_end.charAt(9).toLowerCase() === 'a'
                  ? 'ios-sunny'
                  : 'ios-moon'
              }
              style={{
            ...
            <Text
              ...
              {moment(item?.delivery_date).format('D MMM, YYYY')}
            </Text>
            <Text
              style={{
                color: colors.BUTTON_TEXT_COLOR,
        ...
        <View style={styles.details}>
          ...
          <View style={{flex: 0.8, paddingLeft: '4%'}}>
            ...
            <Text
              ...
              {locality.locality}, {city.city}, {state}, {pincode} IN
            </Text>
          </View>
        </View>
      </View>
      ...
      <View style={styles.innerPaymentModeContainer}>
        <View style={{display: showSubscriptionMenu ? 'flex' : 'none'}}>
          ...
          <View style={styles.subscriptionSlotsContainer}>
            {SUBSCRIPTION_SLOTS.map((item, index) => (
              <View
                key={index}
                style={[
                  styles.subscriptionSlot,
                  {
                    backgroundColor:
                      subscriptionSlot === item.value
                        ? colors.BUTTON_BACKGROUND_COLOR
                        : colors.LIGHT_GRAY_BG_COLOR,
                  },
                ]}>
  ...
  const createOrder = async (
    ...
    let object = {
      ...
      body: {
        ...
        order_total_amount:
          subscriptionSlot > 1
            ? order_total_amount * subscriptionSlot
            : order_total_amount,
    ...
    if (coupon_code) object.body['coupon_id'] = coupon_id;
