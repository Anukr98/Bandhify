...
const AddAddress = ({route}) => {
  ...
  const [latitude, setLatitude] = useState(
    parseFloat(route.params.markerCoordinates.latitude).toFixed(4),
  );
  const [longitude, setLongitude] = useState(
    parseFloat(route.params.markerCoordinates.longitude).toFixed(4),
  );
  ...
  useEffect(() => {
    ...
    const unsubscribe = navigation.addListener('focus', async () => {
      ...
      setAddressLine1(
        route.params.item ? route.params.item.address_line_1 : '',
      );
      setAddressLine2(
        route.params.item ? route.params.item.address_line_2 : '',
      );
      setAddressLine3(
        route.params.item ? route.params.item.address_line_3 : '',
      );
      ...
      postMethod(
        `${urls.GET_CITIES}?with_localities=1`,
        object,
        (err, result) => {
          setIsLoading(false);
  ...
  const getLocalities = async item => {
    ...
    postMethod(
      `${urls.GET_LOCALITIES}?is_active=1&city_id=${item.id}`,
      object,
      (err, result) => {
        if (err) console.log(err);
        else if (result.status && result.code === 200) {
          ...
          setTimeout(() => {
            if (direction === 'Edit' || direction === 'Add')
              ...
              navigation.replace('Checkout', {
                focus: result.address.address_line_1,
              });
          }, 1400);
        } else {
          setIsLoading(false);
  ...
  const editAddress = async () => {
    if (validateErrors()) {
      ...
      postMethod(
        `${urls.UPDATE_ADDRESS}/${route.params.item.id}`,
        object,
        (err, result) => {
          if (err) console.log(err);
          else if (result.status && result.code === 200) {
  ...
  return (
    <View style={{flex: 1, backgroundColor: colors.WHITE}}>
      <View style={{paddingLeft: 8, paddingTop: 7}}>
        <TouchableWithoutFeedback onPress={handleBackButtonClick}>
          <Icon
            active
            type="Ionicons"
            name="arrow-back"
        ...
        <View style={styles.innerHeader}>
                ...
                style={{
                  backgroundColor:
                    addressType === 'Home'
                      ...
                      : 'none',
                  borderLeftColor:
                    addressType === 'Home' ? colors.MANGO_COLOR : 'none',
                  borderLeftWidth: addressType === 'Home' ? 3.2 : 0,
                }}>
                <TouchableWithoutFeedback
                  onPress={() => changeAddressType('Home')}>
                ...
                style={{
                  backgroundColor:
                    addressType === 'Office'
                      ...
                      : 'none',
                  borderLeftColor:
                    addressType === 'Office' ? colors.MANGO_COLOR : 'none',
                  borderLeftWidth: addressType === 'Office' ? 3.2 : 0,
                }}>
                <TouchableWithoutFeedback
                  onPress={() => changeAddressType('Office')}>
                  <View style={styles.addressModalInnerContainer}>
                    <View>
                      <Icon
                        active
                ...
                style={{
                  backgroundColor:
                    addressType === 'Other'
                      ...
                      : 'none',
                  borderLeftColor:
                    addressType === 'Other' ? colors.MANGO_COLOR : 'none',
                  borderLeftWidth: addressType === 'Other' ? 3.2 : 0,
                }}>
                <TouchableWithoutFeedback
                  onPress={() => changeAddressType('Other')}>
                  <View style={styles.addressModalInnerContainer}>
                    <View>
                      <Icon
                        active
          ...
          <View>
            <Text style={styles.addAddressText}>
              {direction === 'Edit' || direction === 'EditFromCheckout'
                ? 'Edit an Address'
                : 'Add a new Address'}
            </Text>
          </View>
          ...
          <View style={styles.formArea}>
            ...
            <TouchableWithoutFeedback
              ...
              <View style={{marginBottom: '4%'}}>
                ...
                <View style={{position: 'absolute', right: 10, top: '25%'}}>
                  <Icon active type="EvilIcons" name="chevron-down" />
                </View>
              </View>
            </TouchableWithoutFeedback>
            ...
            <TouchableWithoutFeedback
              ...
              <View
                style={{
                  display: shouldLocalitiesShow ? 'flex' : 'none',
                  marginBottom: '4%',
                }}>
                <TextInput
                  style={styles.input}
