...
import { View, Text, StyleSheet, FlatList, Image, Platform, TouchableOpacity, BackHandler, LogBox } from 'react-native'
...
const ShopsNear = () => {
    ...
    const geoSuccess = async position => {
        ...
        if (address) {
            ...
            temp.map((item, index) => {
                if (index > 0)
                    secondaryAddress = secondaryAddress + item + ', '
            })
            ...
            .then(async res => {
                let tempAddress = res.results[0].formatted_address, secondaryAddress = ''
                await AsyncStorage.setItem('location',tempAddress)
                await AsyncStorage.setItem('addressLatitude', currentLatitude.toString())
                await AsyncStorage.setItem('addressLongitude', currentLongitude.toString())
    ...
    const getShops = async (lt,long) => {
        ...
        .then(res => {
            ...
            postMethod(`${urls.GET_SHOPS}?latitude=${lt}&longitude=${long}`, object, (err, result) => {
                if (err)
                    console.log(err)
                else if (result.status && result.code === 200) {
                    setIsLoading(false)
    ...
    const getAllProducts = async item => {
        ...
        postMethod(`${urls.GET_PRODUCTS}?selling_products=true&shop_id=${item?.id}&top_selling_products=true`, object, async (err, result) => {
            setTimeout(() => {
                setIsLoading(false)
            },500)
            if (err)
    ...
    const EmptyList = () => (
        <View style = {{ flex : 1 , backgroundColor : colors.WHITE , alignItems : 'center' }}>
            <Image
                ...
                resizeMode = 'contain'
            />
            <View style = {{ width : '93%' , alignItems : 'center' , transform : [{ translateY : -50 }]}}>
                <Text style = {styles.areaNotServiceable}>{strings.AREA_NOT_SERVICEABLE}</Text>
            </View>
        </View>
    )
        ...
        <View>
            <View style={styles.mainBanner}>
                <View style={styles.innerBanner}>
                    <Slider
                        data = {carouselImages}
                        renderItem = {({ item }) => <Image source = {{ uri: item }} style = {{ width: '100%', height: 180, borderRadius: 15 }} resizeMode = "contain" />}
                    ...
                    <View style={{ position: 'relative' }}>
                        <View style={[styles.header]}>
                            <TouchableOpacity
                                style={{ flex: .85 }}
                                activeOpacity={.5}
                                onPress={() => navigation.navigate('SavedAddressesBeforeLocationChange', { latitude, longitude })}
                            >
                                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                    ...
                                    />
                                    <Text style={{ fontFamily: ColorsText.Bold.fontFamily, fontSize: 20, color: 'rgb(0,49,83)' }}>{deliveringAddress.mainAddress}</Text>
                                </View>
                                <View style={{ paddingLeft: '4%', marginTop: -7 }}>
                                    <Text style={{ color: colors.LIGHT_TEXT_COLOR, fontFamily: ColorsText.Medium.fontFamily, fontSize: 12 }} numberOfLines={2}>{deliveringAddress.addressDetails.slice(0, deliveringAddress.addressDetails.length - 2)}</Text>
                                </View>
                            </TouchableOpacity>

                            <TouchableOpacity
                                style={{ flex: .15, alignItems: 'center', paddingTop: '5%', position: 'relative' }}
                                activeOpacity={.5}
                            ...
                            >
                                ...
                                />
                                <View style = {styles.notificationCount}><Text style = {{ color: colors.ACCENT, fontSize: 12 }}>{unreadNotificationCount > 99 ? "99+" : unreadNotificationCount}</Text></View>
                            </TouchableOpacity>
                        </View>
                        {
                            shops.length === 0
